...
function matchWords(dict, wordlist) {
    var result = [];
...
export default {
    ...
    candiFunc(str, flag) {
        ...
        this.visible2 = true
        if (flag == 1) {
            btnlist = btnlist.slice(0, -1);
        }
        btnlist = btnlist.concat(str)
        let code = btnlist.split('').map(ee => a[ee]);
        candidate = matchWords(dict, comb(code))
        pinyincandi = Object.keys(candidate)
        this.candi1 = pinyincandi[0]
        this.candi2 = pinyincandi[1]
        this.candi3 = pinyincandi[2]
        this.candi4 = pinyincandi[3]
        if (pinyincandi[0] == undefined) {
            this.candi1 = ' '
        }
        if (pinyincandi[1] == undefined) {
            this.candi2 = ' '
        }
        if (pinyincandi[2] == undefined) {
            this.candi3 = ' '
        }
        if (pinyincandi[3] == undefined) {
            this.candi4 = ' '
        }
        if (btnlist.length == 1) {
            this.pre = a[btnlist].charAt(1)
        } else if (this.candi1 == ' ') {
            this.pre = btnlist
        } else {
            this.pre = pinyincandi[0]
        }

    },
    sendHanzi(i) {
        candiflag = i
        this.pre = pinyincandi[i]
        this.t1t = dict[pinyincandi[i]].charAt(0)
        this.t2t = dict[pinyincandi[i]].charAt(1)
        this.t3t = dict[pinyincandi[i]].charAt(2)
        this.t4t = dict[pinyincandi[i]].charAt(3)
        this.t5t = dict[pinyincandi[i]].charAt(4)
    },
    btnFun(i) {
        textbox = textbox + i
        storage.set({
    ...
    t1() {
        this.btnFun(this.t1t)
    },
    t2() {
        this.btnFun(this.t2t)
    },
    t3() {
        this.btnFun(this.t3t)
    },
    t4() {
        this.btnFun(this.t4t)
    },
    t5() {
        this.btnFun(this.t5t)
    },
    tmore() {
        if (flag == 0) {
            this.visible3 = false
            ...
            this.list = []
            for (let i = 5; i < 17; i++) {
                this.list.push(dict[pinyincandi[candiflag]].charAt(i))
            }
            this.tmoreic = "/common/ic_down.png"
        } else {
            this.visible3 = true
            this.visible4 = false
            flag = 0
            this.tmoreic = "/common/ic_right.png"
        }
    ...
    right() {
        if (index < dict[pinyincandi[candiflag]].length) {
            this.list = [];
            let i = index;
            for (let j = index; j < i + 12 && j < dict[pinyincandi[candiflag]].length; j++) {
                this.list.push(dict[pinyincandi[candiflag]].charAt(j));
            }
            index = index + 12;
        }
    },
